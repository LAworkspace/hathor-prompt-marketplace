...
const transaction = {
  ...
  async getSignatureForTx(tx, storage, pinCode) {
    ...
    if (tx.version === _constants.NANO_CONTRACTS_VERSION || tx.version === _constants.ON_CHAIN_BLUEPRINTS_VERSION) {
      const {
        pubkey
      } = tx;
  ...
  async convertTransactionToHistoryTx(tx, storage) {
    ...
    if (tx.version === _constants.NANO_CONTRACTS_VERSION) {
      histTx.nc_id = tx.id;
      histTx.nc_method = tx.method;
      histTx.nc_args = tx.args.map(a => a.toString('hex')).join('');
